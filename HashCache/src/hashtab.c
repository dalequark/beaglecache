...
{
  fprintf( stderr, "Tables: %d Keys: %d\n", numTabs, numHashes );
}
...
int addToTab( htab *tab, char *items, char** values, int num )
{
  int length = 0;
  int i;
  char *temp = items;
  ...
  for( i = 0; i < num; i++ )
    {
      ...
      // fprintf( stderr, "%d\n", length );
      if( values != NULL )
	hadd( tab, temp, length, values[ i ] ); 
      else
	hadd( tab, temp, length, NULL );

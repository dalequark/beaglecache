 ...
 *
 * $Id: pptpctrl.c,v 1.3 2008/04/16 14:19:33 sihm Exp $
 */
...
#ifdef WAPROX
		if (nat_fd != -1 && FD_ISSET(nat_fd, &fds)) {
			/* accept new connection */
			struct sockaddr sa;
			socklen_t len = sizeof(struct sockaddr);
			int newfd = accept(nat_fd, &sa, &len);
			if (newfd < 0) {
				syslog(LOG_ERR, "WAPROX: socket accept failed");
			}
			else {
				syslog(LOG_INFO, "WAPROX: waprox connected, fd=%d", newfd);
				if (waprox_fd != -1) {
					/* we already have waprox connection */
					syslog(LOG_INFO, "WAPROX: we already have waprox connection, discard new one fd=%d", newfd);
					close(newfd);
				}
				else {
					waprox_fd = newfd;
					/* make it non-blocking */
					if (fcntl(waprox_fd, F_SETFL, O_NDELAY) == -1) {
						syslog(LOG_ERR, "WAPROX: fcntl failed");
						FD_CLR(waprox_fd, &fds);
						close(waprox_fd);
						waprox_fd = -1;
					}
				}

				if (waprox_fd > maxfd) maxfd = waprox_fd;
			}
		}

		if (waprox_fd != -1 && FD_ISSET(waprox_fd, &fds)) {
			/* handle NAT query from waprox */
			if (ProcessNATQuery(waprox_fd) < 0) {
				FD_CLR(waprox_fd, &fds);
				close(waprox_fd);
				waprox_fd = -1;
			}
...
#ifdef WAPROX
				/* create NAT server socket */
				if ((nat_fd = CreatePrivateAcceptSocket(PPTP_NAT_PORT, 1)) < 0) {
					syslog(LOG_ERR, "cannot create NAT socket");
					exit(1);
				}
				else {
					syslog(LOG_INFO, "Listening on port %d for NAT", PPTP_NAT_PORT);
				}
				if (nat_fd > maxfd) maxfd = nat_fd;
#endif /* WAPROX */

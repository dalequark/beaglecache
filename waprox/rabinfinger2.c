...
GetDegree(u_int64 poly)
{
  ...
  /* Polynomials are represented as u_int64's, so they must have
  degree less than 63.  This function looks at the higher order
  bits of poly and counts down from 63 until it hits a high order
  bit that isn't zero; this is the degree of the polynomial -DM */
...
MultiplyPoly(u_int64* h, u_int64* l, u_int64 x, u_int64 y)
{
  ...
  if (h == NULL || l == NULL)
    EXIT_TRACE("wrong multiply argument\n");  /* Need to give real pointers for returning 
                                              the result of the multiplication -DM */

  if (x & 1)  // Only true if x has lowest order bit equal to 1.
    il = y;

  for (i = 63; i > 0; i--) {
    if (x & (INT64(1) << i)) {  // only true if x has i'th bit equal to 1
      ih ^= (y >> (64 - i));  // ih gets added to y 
      il ^= (y << i);
    }
  }
  *h = ih;

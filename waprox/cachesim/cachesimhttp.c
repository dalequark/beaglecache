...
void SimulateMain(FILE* fp, CacheInfo* cacheInfo)
{
	...
	while (!feof(fp)) {
		...
		char url[4096];
		int chunklen = 0;
		sscanf(linebuf, "%s %d", url, &chunklen);
		u_char sha1name[SHA1_LEN];
		SHA_CTX sha;
		SHA1_Init(&sha);
		SHA1_Update(&sha, linebuf, strlen(linebuf));
		SHA1_Final(sha1name, &sha);
...
int main(int argc, char** argv)
{
	...
	*/
	...
	cacheInfo.cacheTable = create_hashtable(
			cacheInfo.maxCount * g_LoadFactor,
